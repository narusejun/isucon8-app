...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	rows, err := db.Query("SELECT * FROM reservations WHERE event_id = ? AND canceled_at IS NULL ORDER BY reserved_at ASC", eventID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func main() {
	...
	e.GET("/api/users/:id", postUsersID, loginRequired)
	...
	e.GET("/api/events/:id", getApiEventsID)
	e.POST("/api/events/:id/actions/reserve", postApiEventsIDActionsReserve, loginRequired)
	e.DELETE("/api/events/:id/sheets/:rank/:num/reservation", deleteApiEventsIDSheetsRankNumReservation, loginRequired)
	e.GET("/admin/", getAdmin, fillinAdministrator)
	e.POST("/admin/api/actions/login", postAdminApiActionsLogin)
	e.POST("/admin/api/actions/logout", postAdminApiActionsLogout, adminLoginRequired)
	e.GET("/admin/api/events", getAdminApiEvents, adminLoginRequired)
	e.POST("/admin/api/events", postAdminApiEvents, adminLoginRequired)
	e.GET("/admin/api/events/:id", adminApiEventsID, adminLoginRequired)
...
func postUsersID(c echo.Context) error {
	var user User
...
func getApiEventsID(c echo.Context) error {
...
func postApiEventsIDActionsReserve(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func deleteApiEventsIDSheetsRankNumReservation(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func postAdminApiActionsLogin(c echo.Context) error {
	var params struct {
		LoginName string `json:"login_name"`
		Password  string `json:"password"`
	}
...
func postAdminApiActionsLogout(c echo.Context) error {
	sessDeleteAdministratorID(c)
	return c.NoContent(204)
}
func getAdminApiEvents(c echo.Context) error {
	events, err := getEvents(true)
	if err != nil {
		return err
	}
...
func postAdminApiEvents(c echo.Context) error {
	var params struct {
...
func adminApiEventsID(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func postAdminApiEventsIDActionsEdit(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getAdminApiReportsEventsIDSales(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getAdminApiReportsSales(c echo.Context) error {
